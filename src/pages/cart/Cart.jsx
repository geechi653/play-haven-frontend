...
import GameCard from '../../components/GameCard/GameCard.jsx';
import { useGlobalStore } from '../../hooks/useGlobalStore';
// import { fetchUserCart, addToCart, removeFromCart, clearCart } from '../../utils/api'; // Commented out for dummy data
import { IoTrashOutline } from "react-icons/io5";
import './Cart.css';

// Dummy games data for cart - Only 3 games
const dummyCartGames = [
  {
    id: 12,
    title: "Baldur's Gate 3",
    description: "Gather your party and return to the Forgotten Realms in a tale of fellowship and betrayal.",
    image_url: "https://via.placeholder.com/300x400/E84393/FFFFFF?text=Baldurs+Gate+3",
    price: 59.99,
  ...
  },
  {
    ...
    description: "Experience a new story set at Hogwarts in the 1800s. Your character is a student who holds the key to an ancient secret.",
    image_url: "https://via.placeholder.com/300x400/FDCB6E/FFFFFF?text=Hogwarts+Legacy",
    price: 49.99,
  ...
  },
  {
    ...
    image_url: "https://via.placeholder.com/300x400/FF7675/FFFFFF?text=Celeste",
    price: 19.99,
...
// Dummy cart items (with gameId and quantity structure) - Only 3 games
const dummyCartItems = [
  { gameId: 12, quantity: 1 },
  { gameId: 13, quantity: 1 },
  { gameId: 14, quantity: 1 }
];
...
function Cart() {
  ...
  useEffect(() => {
    if (user.isAuthenticated && user.userId) {
      ...
      dispatch({ type: 'SET_CART', payload: { items: dummyCartItems } });
      setCartItems(dummyCartItems);
      
      // Ensure dummy games are available in store
      dispatch({ type: 'SET_GAMES', payload: { allGames: dummyCartGames } });
    }
  }, [user.isAuthenticated, user.userId, dispatch]);
  ...
  const allGames = dummyCartGames;
  const currentCartItems = dummyCartItems;

  const cartGames = allGames.filter(game =>
    user.isAuthenticated &&
    currentCartItems.some(item => item.gameId === game.id) &&
    !removedItems.has(game.id)
  );

  // Dummy implementation for removeFromCart
  const removeItemFromCart = async (gameId) => {
    try {
      // Replace API call with dummy implementation
      // await removeFromCart(user.userId, gameId);
      
      // Simulate API call
      console.log(`Dummy: Removing game ${gameId} from cart`);
  ...
  // Dummy implementation for clearCart
  const clearCartItems = async () => {
    try {
      // Replace API call with dummy implementation
      // await clearCart(user.userId);
      ...
      // Simulate API call
      console.log('Dummy: Clearing entire cart');
  ...
  const handleCheckout = async () => {
    ...
    try {
      // Dummy checkout implementation
      console.log('Dummy: Starting checkout process');
      console.log('Cart items:', cartGames.map(g => g.title));
