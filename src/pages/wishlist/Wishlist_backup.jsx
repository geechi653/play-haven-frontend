...
function Wishlist() {
  ...
  const filteredGames = displayGames.filter(game => {
    ...
    const gameCategories = (game.category || '').split(', ').map(cat => cat.trim());
    const matchesGenre = selectedGenre === 'all' || gameCategories.includes(selectedGenre);
    
    const gamePlatforms = (game.platform || '').split(', ').map(p => p.trim());
    const matchesPlatform = selectedPlatform === 'all' || 
      gamePlatforms.some(p => p.toLowerCase() === selectedPlatform.toLowerCase());
  ...
  const allGenres = [...new Set(displayGames.flatMap(game => 
    (game.category || '').split(', ').map(cat => cat.trim()).filter(Boolean)
  ))];
